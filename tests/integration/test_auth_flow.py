...
from app.data.repositories import PermissionRepository, RoleRepository, UserRepository
...
async def test_login_flow_integration(setup_test_database) -> None:
    ...
    # Create role
    role = Role(name="admin", description="Admin role", permission_ids=[str(perm.id)])
...
async def test_gql_context_integration(setup_test_database):
    ...
    # Create role
    role = Role(
        name="viewer", description="Viewer role", permission_ids=[str(perm_read.id)]
    )
    role = await role_repo.create(role)
