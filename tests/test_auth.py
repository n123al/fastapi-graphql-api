...
from app.data.models.user import User, UserProfile, UserPreferences
...
from app.data.repositories import UserRepository, RoleRepository, PermissionRepository, GroupRepository
from app.services.auth import AuthenticationService
from app.core.auth_strategies import PasswordManager, TokenManager, UsernamePasswordAuthStrategy, AuthorizationService
from app.core.exceptions import AuthenticationError, ValidationError, NotFoundError, TokenError
...
class TestTokenManager:
    ...
    def test_decode_expired_token(self):
        ...
        expired_token = jwt.encode(expired_data, self.token_manager.secret_key, algorithm=self.token_manager.algorithm)
...
class TestUserRepository:
    ...
    async def test_create_user(self):
        ...
        with patch.object(self.repository, 'create', new_callable=AsyncMock) as mock_create:
    ...
    async def test_get_by_email(self):
        ...
        with patch.object(self.repository, 'get_by_email', new_callable=AsyncMock) as mock_get:
            mock_user = Mock(spec=User)
    ...
    async def test_get_by_username(self):
        ...
        with patch.object(self.repository, 'get_by_username', new_callable=AsyncMock) as mock_get:
            mock_user = Mock(spec=User)
...
class TestAuthenticationService:
    ...
    async def test_authenticate_user_success(self):
        ...
        # Mock finding user
        with patch.object(self.auth_service, '_find_user_by_identifier', new_callable=AsyncMock) as mock_find:
            mock_find.return_value = mock_user
            
            # Mock password verification
            with patch('app.services.auth.verify_password', return_value=True):
                # Mock successful login handler
                with patch.object(self.auth_service, '_handle_successful_login', new_callable=AsyncMock):
    ...
    async def test_authenticate_user_invalid_credentials(self):
        ...
        with patch.object(self.auth_service, '_find_user_by_identifier', new_callable=AsyncMock) as mock_find:
            ...
            with patch('app.services.auth.verify_password', return_value=False):
                with patch.object(self.auth_service, '_handle_failed_login', new_callable=AsyncMock):
    ...
    async def test_login_success(self):
        ...
        # Mock authentication
        with patch.object(self.auth_service, 'authenticate_user', new_callable=AsyncMock) as mock_auth:
...
class TestAuthorizationService:
    ...
    async def test_has_permission_with_permission(self):
        ...
        with patch.object(self.auth_service.permission_repository, 'get_by_name', new_callable=AsyncMock) as mock_get:
    ...
    async def test_has_role_with_role(self):
        ...
        with patch.object(self.auth_service.role_repository, 'get_by_id', new_callable=AsyncMock) as mock_get:
    ...
    async def test_has_all_permissions_with_all_permissions(self):
        ...
        with patch.object(self.auth_service.permission_repository, 'get_by_name', side_effect=mock_get_by_name):
