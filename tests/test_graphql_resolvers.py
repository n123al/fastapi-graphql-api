...
from app.graphql.types.types import LoginInput, UserInput
...
class TestUserQueries:
    ...
    async def test_user_query_other_data_authorized(self):
        ...
        mock_admin = DBUser(
            username="admin", email="admin@example.com", hashed_password="hashed"
        )
        mock_admin.id = "admin123"

        mock_target = DBUser(
            username="target", email="target@example.com", hashed_password="hashed"
        )
    ...
    async def test_user_query_other_data_unauthorized(self):
        ...
        mock_user = DBUser(
            username="user", email="user@example.com", hashed_password="hashed"
        )
        mock_user.id = "user123"
...
class TestAuthMutations:
    ...
    async def test_login_success(self):
        ...
        with patch(
            "app.graphql.mutations.auth_mutations.AuthenticationService"
        ) as MockAuthService:
            mock_service_instance = AsyncMock()
            mock_service_instance.authenticate_user = AsyncMock(return_value=mock_user)
            mock_service_instance.generate_tokens = AsyncMock(
                return_value={
                    "access_token": "access",
            ...
            result = await resolver._auth_service.authenticate_user(
                login_input.identifier, login_input.password
            )
...
class TestGroupQueries:
    ...
    async def test_group_by_name_member(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.graphql.queries.group_queries.get_current_user",
...
class TestUserMutations:
    ...
    async def test_create_user_mutation(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.graphql.mutations.user_mutations.UserService", FakeUserService
        )
        info = Mock()
        info.context = Mock()
        info.context.has_permission = AsyncMock(return_value=True)
    ...
    async def test_update_user_mutation_self(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.graphql.mutations.user_mutations.get_current_user",
            AsyncMock(return_value=FakeUser("uid2")),
        )
        info = Mock()
        info.context = Mock()
        info.context.has_permission = AsyncMock(return_value=True)
    ...
    async def test_deactivate_user_mutation_self(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.graphql.mutations.user_mutations.get_current_user",
            AsyncMock(return_value=FakeUser("uid5")),
        )
        info = Mock()
        info.context = Mock()
        info.context.has_permission = AsyncMock(return_value=True)
