...
from app.graphql.auth import require_permission, require_any_permission, get_current_user
...
class GroupQueries:
    ...
    async def groups(self, info: Info, limit: int = 100) -> List[Group]:
        ...
        try:
            ...
            if await info.context.has_permission("groups:read"):
                ...
                user_groups = current_user.profile.groups if current_user.profile.groups else []
                if not user_groups:
                    return []
    ...
    async def system_groups(self, info: Info) -> List[Group]:
        ...
        try:
            ...
            collection = motor_db_manager.database["groups"]
            docs = await collection.find({"is_system_group": True, "is_active": True}).to_list(None)
