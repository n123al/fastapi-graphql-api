...
class UserService(BaseService[User]):
    ...
    async def update_user(
        ...
        if "username" in update_data:
            ...
            if new_username != existing_user.username:
                ...
                username_conflict = await self.user_repository.get_by_username(
                    new_username
                )
        ...
        if "preferences" in update_data:
            ...
            if isinstance(preferences_updates, dict):
                ...
                updated_preferences = {
                    **existing_user.preferences.dict(),
                    **preferences_updates,
                }
        ...
        if updated_user:
            self._log_operation(
                "user_updated",
                {"user_id": user_id, "updates": list(processed_updates.keys())},
            )
    ...
    async def deactivate_user(self, user_id: str) -> Optional[User]:
        ...
        return await self.update_user(
            user_id, {"is_active": False, "deactivated_at": datetime.now(timezone.utc)}
        )
    ...
    async def activate_user(self, user_id: str) -> Optional[User]:
        ...
        """
        return await self.update_user(
            user_id, {"is_active": True, "deactivated_at": None}
        )
