...
class Permission(BaseDataModel):
    ...
    description: Optional[str] = Field(default=None, max_length=500, description="Permission description")
    
    # Resource and action specification
    resource: Optional[str] = Field(default=None, max_length=50, description="Resource this permission applies to")
    action: Optional[str] = Field(default=None, max_length=50, description="Action this permission grants")
    
    # System permission flag
    is_system_permission: bool = Field(default=False, description="Whether this is a system permission that cannot be deleted")
    ...
    def can_perform(self, resource: str, action: str) -> bool:
        ...
        """
        return (self.resource == resource and self.action == action) if (self.resource and self.action) else False
    ...
    def __repr__(self) -> str:
        ...
        resource_action = f"{self.resource}:{self.action}" if (self.resource and self.action) else self.name
