...
class Group(BaseDataModel):
    ...
    group_type: str = Field(default="general", description="Type of group (department, team, project)")
    ...
    settings: Dict[str, Any] = Field(default_factory=dict, description="Group configuration settings")
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Additional metadata")
    
    # Access control
    max_members: Optional[int] = Field(default=None, description="Maximum number of members")
    is_public: bool = Field(default=False, description="Whether group is publicly discoverable")
    requires_approval: bool = Field(default=True, description="Whether membership requires approval")
    
    # Timestamps for group lifecycle
    created_at: Optional[datetime] = Field(default=None, description="Group creation timestamp")
    ...
    def validate_name(cls, v: str) -> str:
        ...
        if not v.replace('-', '').replace('_', '').isalnum():
            raise ValueError("Group name must be alphanumeric with hyphens and underscores only")
    ...
    def validate_group_type(cls, v: str) -> str:
        """Validate group type."""
        valid_types = {'general', 'department', 'team', 'project', 'organization', 'system'}
    ...
    def add_member(self, user_id: str) -> bool:
        ...
        if self.max_members and len(self.member_ids) >= self.max_members:
            raise ValueError(f"Group has reached maximum capacity of {self.max_members} members")
    ...
    def to_dict(self, exclude_none: bool = True, exclude_sensitive: bool = True) -> Dict[str, Any]:
        """
        ...
        """
        data = self.model_dump(exclude_none=exclude_none) if hasattr(self, 'model_dump') else self.dict(exclude_none=exclude_none)
