...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    try:
        ...
        logger.warning(
            "Continuing startup in degraded mode without database connection"
        )
    try:
        await setup_graphql()
    except Exception as e:
...
async def http_exception_handler(request: Request, exc: HTTPException) -> JSONResponse:
    ...
    return JSONResponse(
        ...
        content={"error": {"message": exc.detail, "code": f"HTTP_{exc.status_code}"}},
    )
...
async def setup_graphql() -> None:
    ...
    try:
        ...
        logger.info(
            "GraphQL setup completed with Strawberry's built-in GraphiQL Playground"
        )
        logger.info(
            f"Visit http://{settings.HOST}:{settings.PORT}/api/graphql to access the GraphQL Playground"
        )
...
async def detailed_health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        health_status["features"]["database_connected"] = db_health.get(
            "connected", False
        )
        ...
        if db_health.get("connected") and db_health.get("query_ok"):
            if latency >= 800:
                ...
                try:
                    ...
                    logger.warning(
                        "Failed to get database stats", error=str(stats_error)
                    )
        else:
            health_status["services"]["database"] = "unhealthy"
            health_status["status"] = (
                "unhealthy" if db_health.get("error") else "degraded"
            )
    except Exception as e:
        health_status["services"]["database"] = "unhealthy"
