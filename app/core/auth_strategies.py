...
class ITokenManager(ABC):
    ...
    @abstractmethod
    def create_action_token(self, data: Dict[str, Any], minutes: int, token_type: str) -> str:
        pass
...
class TokenManager(ITokenManager):
    ...
    def decode_token(self, token: str) -> Dict[str, Any]:
        """Decode and validate a token."""
        try:
            payload = cast(Dict[str, Any], jwt.decode(token, self.secret_key, algorithms=[self.algorithm]))
            return payload
...
class UsernamePasswordAuthStrategy(IAuthenticationStrategy):
    ...
    async def authenticate(self, credentials: Dict[str, Any]) -> User:
        ...
        user: Optional[User] = await self.user_repository.get_by_email_or_username(identifier)
        if not user:
