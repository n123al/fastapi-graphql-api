...
def sanitize_html(raw_html: str, allowed_tags: Optional[list] = None) -> str:
    ...
    if allowed_tags is None:
        allowed_tags = ['p', 'br', 'strong', 'em', 'u', 'i', 'b', 'ul', 'ol', 'li', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6']
...
def send_email_smtp(to: str, subject: str, body: str) -> bool:
    try:
        ...
        use_tls = bool(settings.SMTP_USE_TLS) if settings.SMTP_USE_TLS is not None else (port == 587)
        use_ssl = bool(settings.SMTP_USE_SSL) if settings.SMTP_USE_SSL is not None else (port == 465)
