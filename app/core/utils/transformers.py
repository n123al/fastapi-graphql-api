...
def serialize_datetime(dt: Union[datetime, None], format_string: str = "%Y-%m-%dT%H:%M:%S.%fZ") -> Union[str, None]:
    """
...
def deserialize_datetime(dt_str: Union[str, None], format_string: str = "%Y-%m-%dT%H:%M:%S.%fZ") -> Union[datetime, None]:
    """
...
def transform_keys_recursive(data: Any, transform_func: Callable[[str], str]) -> Any:
    ...
    if isinstance(data, dict):
        return {transform_func(key): transform_keys_recursive(value, transform_func) for key, value in data.items()}
