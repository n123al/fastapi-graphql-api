...
class SecurityManager:
    ...
    async def get_current_active_user(
        self, current_user: Any = Depends(get_current_user)
    ) -> Any:
    ...
    def require_permission(self, permission: str) -> Callable[..., Any]:
        ...
        async def permission_checker(
            current_user: Any = Depends(self.get_current_user),
        ) -> Any:
...
def create_set_password_token(data: dict) -> str:
    ...
    token_manager = TokenManager()
    return token_manager.create_action_token(
        data, settings.SET_PASSWORD_TOKEN_EXPIRE_MINUTES, "set_password"
    )
